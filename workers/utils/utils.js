/**
 * 将一个数组按某项分类 并将分类后的值转换为source里对应的值
 * @param arr {Array} 需被分类的数组
 * @param index {Number} 分类条件的索引
 * @param source {Array} 将初步分类后的index替换为有意义的内容
 * @returns {{}} 返回分类后的键值对 例如将所有airports分类得到"country" => [index] 再将index根据allAirportsPos转换为对应坐标
 * @example
 * [
 *  ["a","b","c"],
 *  ["e","b","f"],
...
// 功能与divideArrayByItem相似 不同在于返回的结果里包含索引idx airport分类专用
function divideArrayByItemWithIdx(arr,index,source){
    let result = {};
    arr.forEach((item,idx) => {
        let condition = item[index];
        if(result[condition]){
